...
$catalogue = @(
    ...
    New-Endpoint 'join.secure.skypeassets.com'        $WebPorts  'Core Teams' 'Both' 'Commercial' 'Teams/Skype asset delivery'
    New-Endpoint 'mlccdnprod.azureedge.net'           $WebPorts  'Core Teams' 'Both' 'Commercial' 'Teams meeting/live-event CDN'
    ...
    New-Endpoint 'licensing.mp.microsoft.com'       $StorePorts 'Microsoft Store' 'Windows' 'Commercial' 'Store licensing (AppInstallManager)'
    ...
    New-Endpoint 'wustat.windows.com'            $HttpPort   'Windows Update' 'Windows' 'Commercial' 'Windows Update statistics'
...
foreach ($fnName in 'Test-Dns', 'Test-TcpPort', 'Test-Tls12') {
...
$probe = {
    ...
    [pscustomobject]@{ Dns = $dns; Ports = @($portResults); Tls = $tls }
}
...
try {
    ...
    foreach ($job in $jobs) {
        ...
        $portsOk = @($portResults | Where-Object { $_.Success }).Count
        $overall =
            if (-not $dns.Success) { 'Fail' }
            elseif ($portsOk -eq 0) { 'Fail' }
            elseif ($tls -and (-not $tls.Success -or -not $tls.ModernTls)) { 'Fail' }
            elseif ($portsOk -lt $ep.Ports.Count) { 'Partial' }
            else { 'Pass' }
        ...
        $color = switch ($overall) { 'Pass' {'Green'} 'Partial' {'Yellow'} default {'Red'} }
...
Write-Host ("-" * 60)
Write-Host ("Summary: {0} pass, {1} partial, {2} fail" -f $pass, $partial, $fail) -ForegroundColor Cyan
...
function Get-ResultRows {
    ...
    $out = foreach ($r in ($Items | Sort-Object Platform, Category, Host)) {
        $badgeClass = switch ($r.Overall) { 'Pass' {'ok'} 'Partial' {'warn'} default {'bad'} }

        $portCells = foreach ($p in $r.Ports) {
            $cls = if ($p.Success) { 'ok' } else { 'bad' }
            "<span class='pill $cls' title='$(HtmlEncode $p.Detail)'>$($p.Port)</span>"
        }
...
if ($fail -eq 0 -and $partial -eq 0) {
    if ($inspDetected.Count -eq 0 -and $inspSuspected.Count -eq 0) {
    ...
    $dnsFails = $results | Where-Object { -not $_.Dns.Success }
    if ($dnsFails) {
    ...
    $portBlocked = $results | Where-Object {
        $_.Dns.Success -and (@($_.Ports | Where-Object { -not $_.Success }).Count -gt 0)
    }
    if ($portBlocked) {
        $blockedList = foreach ($b in $portBlocked) {
            $bp = (@($b.Ports | Where-Object { -not $_.Success }).Port) -join ', '
            "$($b.Host) (port $bp)"
        }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .card.fail .n { color:#c50f1f; } .card.total .n { color:#5059c9; }
  ...
  .pill.ok{background:#dff6dd;color:#0b6a0b;} .pill.bad{background:#fde7e9;color:#a4262c;}
  .pill.warnpill{background:#fff4ce;color:#8a6d00;}
  .ok-text{color:#107c10;font-weight:600;} .bad-text{color:#c50f1f;font-weight:600;}
  .muted{color:#999;}
